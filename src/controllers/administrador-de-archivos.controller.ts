...
export class AdministradorDeArchivosController {
  ...
  @post('/cargar-archivo-usuario', {
    responses: {
      200: {
        ...
        description: 'foto de usuario a cargar',
      },
    },
  })
  async CargarArchivoUsuario(
    ...
    const filePath = path.join(
      __dirname,
      ConfiguracionGeneral.carpetaArchivosUsuarios,
    );
    let res = await this.StoreFileToPath(
      filePath,
      ConfiguracionGeneral.campoUsuario,
      request,
      response,
  ...
  async CargarArchivoConductor(
    ...
    const filePath = path.join(
      __dirname,
      ConfiguracionGeneral.carpetaArchivosConductores,
    );
    let res = await this.StoreFileToPath(
      filePath,
      ConfiguracionGeneral.campoConductor,
      request,
      response,
      ConfiguracionGeneral.extensionesImagenes,
    );
